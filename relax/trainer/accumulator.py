...
class SampleLog:
    __slots__ = ("sample_step", "sample_episode", "episode_return", "episode_length", "last_episode_return", "accumulator")
    ...
    def add(self, reward: float, terminated: bool, truncated: bool, info: dict):
        ...
        if done:
            ...
            if self.episode_length > 0:
                self.accumulator.add("mean_step_reward", float(self.episode_return) / self.episode_length)
            self.last_episode_return = float(self.episode_return)
            self.episode_return = 0.0
            self.episode_length = 0
...
class VectorSampleLog:
    __slots__ = ("num_envs", "sample_step", "sample_episode", "episode_return", "episode_length", "last_episode_returns", "accumulator")
    ...
    def add(self, reward: np.ndarray, terminated: np.ndarray, truncated: np.ndarray, info: dict):
        ...
        self.last_episode_returns = self.episode_return[done].tolist()
        done_lengths = self.episode_length[done]
        done_returns = self.episode_return[done]
        safe = done_lengths > 0
        if safe.any():
            self.accumulator.add_vec("mean_step_reward", (done_returns[safe] / done_lengths[safe]).tolist())
