...
class Algorithm:
    ...
    def save_policy_structure(self, root: os.PathLike, dummy_obs: jax.Array) -> None:
        ...
        # Save head-0-only deterministic policy for comparison (ensemble methods only)
        if hasattr(self.agent, 'get_deterministic_action_head0'):
            det_head0 = make_persist(self.agent.get_deterministic_action_head0)(self.get_policy_params(), dummy_obs)
            det_head0.save(root / "deterministic_head0.pkl")
            det_head0.save_info(root / "deterministic_head0.txt")
