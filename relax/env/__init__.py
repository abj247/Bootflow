...
# MetaWorld task names for detection
METAWORLD_TASKS = {
    "reach-v3", "drawer-close-v3", "door-open-v3", "button-press-topdown-v3",
    "peg-insert-side-v3", "shelf-place-v3", "sweep-into-v3",
}


def _is_metaworld(name: str) -> bool:
    return name in METAWORLD_TASKS or name.endswith("-v3")


def create_metaworld_env(name: str, seed: int, action_seed: int = 0):
    """Create a single MetaWorld environment wrapped for this codebase."""
    import metaworld
...
def create_env(name: str, seed: int, action_seed: int = 0):
    if _is_metaworld(name):
        return create_metaworld_env(name, seed, action_seed)
    env = make(name)
    env.reset(seed=seed)
    env = RelaxWrapper(env, action_seed)
    return env, env.obs_dim, env.act_dim
