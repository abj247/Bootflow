...
class BootflowExperience(NamedTuple):
    obs: "jax.Array"
    action: "jax.Array"
    reward: "jax.Array"
    done: "jax.Array"
    next_obs: "jax.Array"
    bootstrap_mask: "jax.Array"   # (K,) binary mask per transition
    active_head: "jax.Array"      # scalar int, which head generated this action

    def batch_size(self) -> Optional[int]:
        return probe_batch_size(self.reward)

    def __repr__(self):
        return f"BootflowExperience(size={self.batch_size()})"

    @staticmethod
    def create_example(obs_dim: int, action_dim: int, num_heads: int, batch_size: Optional[int] = None):
        leading_dims = (batch_size,) if batch_size is not None else ()
        return BootflowExperience(
            obs=np.zeros((*leading_dims, obs_dim), dtype=np.float32),
            action=np.zeros((*leading_dims, action_dim), dtype=np.float32),
            reward=np.zeros(leading_dims, dtype=np.float32),
            next_obs=np.zeros((*leading_dims, obs_dim), dtype=np.float32),
            done=np.zeros(leading_dims, dtype=np.bool_),
            bootstrap_mask=np.ones((*leading_dims, num_heads), dtype=np.float32),
            active_head=np.zeros(leading_dims, dtype=np.int32),
        )

    @staticmethod
    def create(obs, action, reward, terminated, truncated, next_obs, info=None,
               bootstrap_mask=None, active_head=0):
        if bootstrap_mask is None:
            bootstrap_mask = np.ones(1, dtype=np.float32)  # placeholder
        return BootflowExperience(
            obs=obs, action=action, reward=reward, done=terminated,
            next_obs=next_obs, bootstrap_mask=bootstrap_mask,
            active_head=np.int32(active_head),
        )
